...
int testWASD(const uav_msgs::MissionParams &m, ros::NodeHandle &nh)
{
	...
	geometry_msgs::Twist velocity_msg; // used to store setpoint msgs
...
int precLand(const uav_msgs::MissionParams &m, ros::NodeHandle &nh)
{
	...
	ros::ServiceClient envClient = nh.serviceClient<uav_msgs::DetectObject>("/odroid/env_percept/detectObject"); //Lilo changed raspi to odroid
	...
	ros::Subscriber sendPrecLand = nh.subscribe<uav_msgs::PrecLand>("/odroid/image_percept/landing_target", 1, sendPrecLandCb); //Lilo changed raspi to odroid
	ros::AsyncSpinner spinner(1);
	while (!called) {
		spinner.start();
...
int loiterDZ(const uav_msgs::MissionParams &m, ros::NodeHandle &nh)
{
	...
	double last_called;
	boost::function<void(const std_msgs::Float64 &)> x_cb = [&] (const std_msgs::Float64 &msg) {
		if (fabs(msg.data) < 1.8)
			off_x = msg.data;
		else {
			ROS_ERROR_THROTTLE(0.5, "Dist X > 1.8 meter oyy. Now sending zero");
			off_x = 0.0;
		}			
		called1 = true;
	...
	int lost_count = 0;
	double last_y_called;
	boost::function<void(const sensor_msgs::Range &)> y_cb = [&] (const sensor_msgs::Range &msg) {
		if (msg.range > 0.3 && msg.range < 10)
			off_y = want_y - msg.range;
		else
			lost_count++;
		if (lost_count >= 4) {
			off_y = 0.0;
			ROS_INFO_ONCE("Lost count exceeded 4. Off y will be zero.");
		}
		called2 = true;
		last_y_called = ros::Time::now().toSec();
	};
	double now_alt, x_tol, y_tol;
	x_tol = config.doubles.at("PREC_X_TOL");
	y_tol = config.doubles.at("PREC_Y_TOL");
	bool reached = false;
	boost::function<void(const sensor_msgs::Range &)> z_cb = [&] (const sensor_msgs::Range &msg) {
		now_alt = msg.range;
		called3 = true;
		if (called1 && called2 && off_x < x_tol && off_y < y_tol && !debug)
			reached = true;
		else reached = false;
	};
	...
	boost::array<uint16_t, 18U> precLoiterData {65535, 65535, 1500, 65535, 65535,
		2100, 65535, 65535};
	precLoiterMsg.channels = precLoiterData; 
...
/*	mavros_msgs::RCIn rcStatus;
	...
	while (true) {
		mavPrecMsg.size.at(0) = 0.5;
		mavPrecMsg.size.at(1) = 0.5;
		mavPrecMsg.angle.at(0) = std::atan(off_x / now_alt);
		mavPrecMsg.angle.at(1) = std::atan(off_y / now_alt);
		mavPrecMsg.distance = sqrt(pow(off_x, 2) + pow(off_y, 2) + pow(now_alt, 2));
		mavPrecLoiter.publish(mavPrecMsg);
		precLoiterPub.publish(precLoiterMsg);
		freq.sleep();
		if (reached) {
			if (ros::Time::now().toSec() - prevTime > 2.5)
				break;
		}
		else {
		...
		if (ros::Time::now().toSec() - last_called > 1.0 && called1) {
			off_x = 0.0;
			ROS_ERROR_ONCE("Not seeing dropzone. Set off x to zero");
			x_sub.shutdown();
			checkX = false;
		}
		}
		if (checkY) {
		if (ros::Time::now().toSec() - last_y_called > 0.25 && called2) {
			off_y = 0.0;
			for (uint8_t i = 0; i < 10; i++) {
				ROS_ERROR("Not getting RF DEPAN messages. Off y set to zero");
				y_sub.shutdown();
				checkY = false;
			}
		}
